...
namespace VMPlex
{
    class VMManager
    {
        ...
        {
            // Disabled: Renaming VM functionality
            UI.MessageBox.Show(
                System.Windows.MessageBoxImage.Information,
                "VM Rename Disabled",
                "VM renaming functionality has been disabled.",
        ...
        {
            // Disabled: Creating new snapshots
            UI.MessageBox.Show(
                System.Windows.MessageBoxImage.Information,
                "Snapshot Creation Disabled",
                "Creating new snapshots has been disabled.",
                System.Windows.MessageBoxButton.OK);
            return;
        ...
        public static void RevertSnapshot(VirtualMachine vm)
        {
            ...
            // Get all snapshots and find the first one (earliest by creation time)
            var allSnapshots = system.GetAssociated<IMsvm_VirtualSystemSettingData>("Msvm_SnapshotOfVirtualSystem");
            
            if (allSnapshots == null || !allSnapshots.Any())
            {
                UI.MessageBox.Show(
                    System.Windows.MessageBoxImage.Information,
                    "No Snapshots",
            ...
            // Find the first snapshot (earliest creation time)
            IMsvm_VirtualSystemSettingData firstSnapshot = allSnapshots
            ...
            // Verify this is indeed the first snapshot
            var snapshotList = allSnapshots.ToList();
            if (snapshotList.Count > 1)
            {
                UI.MessageBox.Show(
                    System.Windows.MessageBoxImage.Information,
                    "Reverting to First Snapshot",
                    $"Reverting to the first snapshot (created on {firstSnapshot.CreationTime}). Other snapshots are available but only the first can be reverted to.",
                    System.Windows.MessageBoxButton.OK);
            }

            ApplySnapshot(new Snapshot(firstSnapshot, false));
        }
        ...
        public static void ApplySnapshot(Snapshot snapshot)
        {
            ...
            if (allSnapshots != null && allSnapshots.Any())
            {
                ...
                if (firstSnapshot == null || snapshot.SettingData.InstanceID != firstSnapshot.InstanceID)
                {
                    UI.MessageBox.Show(
                        System.Windows.MessageBoxImage.Error,
                        "Snapshot Revert Restricted",
                        "Only reverting to the first snapshot is allowed.",
                        System.Windows.MessageBoxButton.OK);
                    return;
                }
            }
        ...
        {
            // Disabled: Deleting snapshots
            UI.MessageBox.Show(
                System.Windows.MessageBoxImage.Information,
                "Snapshot Deletion Disabled",
                "Snapshot deletion functionality has been disabled.",
                System.Windows.MessageBoxButton.OK);
            return;
        ...
        {
            // Disabled: Deleting snapshot trees
            UI.MessageBox.Show(
                System.Windows.MessageBoxImage.Information,
                "Snapshot Deletion Disabled",
                "Snapshot deletion functionality has been disabled.",
        ...
        // Note: VM creation functionality is not present in this file
        // VM deletion functionality is not present in this file
        // These would need to be disabled in the UI layer or other parts of the codebase
        ...
        {
            // Note: New VM creation via WMI events is still monitored
            // but actual VM creation should be disabled in the UI/other parts
        ...
        {
            // Note: VM deletion via WMI events is still monitored
            // but actual VM deletion should be disabled in the UI/other parts
            IMsvm_ComputerSystem target = e.TargetInstance;
